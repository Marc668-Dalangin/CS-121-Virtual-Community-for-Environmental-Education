...
if (isset($_GET['token'])) {
    ...
    if ($reset) {
        ...
        if ($currentTime > $expiresAt) {
            header("Location: forgot_password_student.php?error=expired");
            exit;
        }
        ...
        if (isset($_POST['new_password']) && isset($_POST['confirm_password'])) {
            ...
            if (!preg_match($passwordPattern, $newPassword)) {
                ...
                $studentStmt = $pdo->prepare("SELECT password FROM students WHERE email = :email");
                $studentStmt->execute(['email' => $emailOrUsername]);
                $student = $studentStmt->fetch();

                if ($student && password_verify($newPassword, $student['password'])) {
                    $error_message = "New password cannot be the same as the old password.";
                } else {

                    $hashedPassword = password_hash($newPassword, PASSWORD_DEFAULT);

                    $updateStmt = $pdo->prepare("UPDATE students SET password = :password WHERE email = :email");
